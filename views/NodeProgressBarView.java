 ...
 */
public class NodeProgressBarView extends View implements Runnable {
     ...
     * 蓝色进度条高度
     */
    private int blueProgressHeight = 30;
    ...
    public NodeProgressBarView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    public void run() {
        //读取View宽度
        viewWidth = NodeProgressBarView.this.getWidth();
        //读取View高度
        viewHeight = NodeProgressBarView.this.getHeight();
        // 进度条宽度计算
...
//            canvas.drawText(str, x-textWidht/2, y+offTextY, paint);
            ...
            //换行处理
            StaticLayout layout = new StaticLayout(str, textPaint, (int) textWidht,
                    Layout.Alignment.ALIGN_CENTER, 1.0F, 0.0F, true);
            canvas.save();
            //画的位置
            canvas.translate(x - textWidht / 2, y + offTextY);
            layout.draw(canvas);
            canvas.restore();
        }
