...
public class ArcProgressbar extends View {
    ...
    private void init(Canvas canvas) {
        ...
        mPaintCircle.setColor(bgColor);//设置画笔的颜色

        mPaintDroitBg = new Paint();
        mPaintDroitBg.setAntiAlias(true);//抗锯齿

        System.out.print("进度的值为" + progress);
        //进度不同状态时边缘显示的颜色
        if (progress == 0) {
        ...
        canvas.drawCircle(
                ...
                bgStrokeWidth / 2, mPaintDroitBg);// 小圆
...
//设置颜色渐变效果
//        RadialGradient radialGradient=new RadialGradient((float) (cx1 + arcRadius
//                * Math.cos(angleOfMoveCircle * 3.14 / 180)),
//                (float) (cy1 + arcRadius
//                        * Math.sin(angleOfMoveCircle * 3.14 / 180)),
//                bgStrokeWidth / 2,Color.RED, Color.GREEN,
//        Shader.TileMode.MIRROR);//设置颜色渐变
//        mPaintBar.setShader(radialGradient);
